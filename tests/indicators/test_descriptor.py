...
def test_rejects_numeric_coarse_value_outside_bounds() -> None:
    with pytest.raises(
        ValueError,
        match="coarse value must be within the parameter space",
    ):
        NumericParameterSpace(
            minimum=0.0,
            maximum=100.0,
...
def test_rejects_numeric_default_outside_bounds() -> None:
    with pytest.raises(
        ValueError,
        match="default must be within the parameter space",
    ):
        NumericParameterSpace(
            minimum=0.0,
            maximum=100.0,
...
def test_rejects_inverted_integer_parameter_bounds() -> None:
    with pytest.raises(
        ValueError,
        match=r"minimum must be less than or equal to maximum\.",
    ):
        IntegerParameterSpace(
            minimum=50,
            maximum=5,
...
def test_rejects_negative_integer_parameter_step() -> None:
    with pytest.raises(
        ValueError,
...
def test_rejects_non_integer_minimum() -> None:
    with pytest.raises(
        TypeError,
        match="minimum must be an integer",
    ):
        IntegerParameterSpace(
            minimum=5.5,
            maximum=50,
