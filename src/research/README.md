...
3. `InMemoryMarketExperimentSpecificationResolver` находит явную регистрацию каждого элемента плана.
4. `ResearchCampaignPlanMarketAdapter` проверяет соответствие инструмента и таймфрейма.
5. `RunMarketResearchCampaign` завершает разрешение всего плана до первого запуска.
6. Каждый результат сохраняет связь с исходным `CampaignExperimentSpecification`.
